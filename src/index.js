...
bot.on('message', message => { //runs when event "message" is sent (so when the bot sees msgs)
    ...
    if (!message.member.roles.cache.has(config.staffid) && !message.member.roles.cache.has(config.boosterrole)) return message.channel.send("Sorry but my commands are for Server boosters only!")
